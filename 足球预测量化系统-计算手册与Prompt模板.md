...
> 一份可复现的足球预测系统工程指南。
> 涵盖：7 个模型的完整公式 + 推理逻辑 + 权重设计 + 风险标签触发 + Prompt 模板 + 输出格式
...
Step 12: 双大比分模型检测
  模型A(≥6球) + 模型B(4-5球) → 动态选择A/B/不激活

Step 13: 6层融合
  标准模型 85% + 大比分模型 15% (条件激活)
...
| 数据项 | 来源 | 说明 |
|--------|------|------|
| 历史比赛 | SQLite `local_stage2.db` 或 PostgreSQL | 11,011 场，2015-2025 |
...
| FIFA 层级 | 代码硬编码 | 按FIFA排名分 Tier 1-4 |
...
标准泊松模型天生低估尾部概率。当强队打出7:1或4:1等大比分时，标准模型预测的 λ 只有 2-3，完全无法覆盖 4+ 进球场景。

**解决方案**：基于世界杯历史比赛（1930-2022年，共计964场）的比分分布统计，设计两个独立的大比分模型：
- **模型A (本模型)**：复合泊松-伽马，专门预测总进球 ≥ 6 的极端大比分
- **模型B**：复合泊松-λ，专门预测总进球 4-5 的中等大比分

### 16.2 世界杯历史数据基准

基于1930-2022年历届世界杯共计964场比赛的比分分布统计：
...
Layer 6: 双大比分模型协调器
  ...
  │
  ▼
Final Fusion: 标准模型 85% + 大比分模型 15% (条件激活)
  │
  ▼
